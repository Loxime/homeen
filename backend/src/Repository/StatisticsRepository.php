...
SELECT
    COALESCE(
        SUM(slice.active_seconds),
        0
    )
FROM app_usage_slice slice
INNER JOIN app_usage_session session
    ON session.id =
        slice.session_id
WHERE session.user_id = :userId
  AND slice.occurred_at >= :start
  AND slice.occurred_at < :end
SQL,
                    $params,
                );
...
SQL,
     ...
     * }
     */
    private function pomodoroMetrics(
        \DateTimeImmutable $start,
        \DateTimeImmutable $end,
    ): array {
        $sessions =
     ...
     * }>
     */
    private function pomodoroDaily(
        \DateTimeImmutable $start,
        \DateTimeImmutable $end,
    ): array {
        $sessions =
...
SQL,
     ...
     */
    private function pomodoroOverlap(
        array $session,
